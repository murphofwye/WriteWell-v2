// --- Constants and State ---
const defaultSettings = { work: 25, break: 5, sprints: 3 }; // minimum sprints is 3 now!
...
// --- Corrected work/break/sprint logic
function handleTimerEnd() {
  ...
  if (isWork) {
    ...
    // Break finished, advance to next work sprint (do not increment sprintIndex here)
    isWork = true;
    timeLeft = settings.work * 60;
    updateTimerDisplay();
    startBtn.disabled = true;
    pauseBtn.disabled = false;
    resetBtn.disabled = true;
    startTimer(); // auto-start next work
  }
}
...
saveWordBtn.addEventListener('click', () => {
  ...
  sprintIndex += 1; // Only increment after WORK is finished!
...
function saveSettingsToProject() {
  if (!currentProject || !projects[currentProject]) return;
  // Minimum sprints is 3
  const work = Math.max(1, parseInt(workLengthInput.value, 10) || 25);
  const brk = Math.max(1, parseInt(breakLengthInput.value, 10) || 5);
  const sprints = Math.max(3, parseInt(sprintCountInput.value, 10) || 3);
  sprintCountInput.value = sprints; // update the input to reflect the new min
